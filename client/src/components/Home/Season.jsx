...
const Season = () => {
    ...
    useEffect(() => {
        ...
        if (seasonSection) {
            observer.observe(seasonSection);
        }

        return () => {
            if (seasonSection) {
                observer.unobserve(seasonSection);
            }
        };
    }, []);

    return (
        <div id='season' className=' bg-gray-900 text-white min-h-screen'>
            <div className='flex flex-col justify-center items-center' style={{ animation: isVisible ? 'fadeIn 1s ease-out' : 'none' }}>
                <h1 className="text-2xl md:text-4xl font-mono tracking-wider text-center mt-10"> - Seasonal Beauty - </h1>
                <p className="text-lg lg:text-xl m-8 text-justify" style={{ animation: isVisible ? 'slideIn 1s ease-out 0.4s forwards' : 'none' }}>
                ...
                <div className={`flex flex-col md:flex-row justify-center items-center m-4`} style={{ animation: isVisible ? 'slideInLeft 2s ease-out forwards' : 'none' }}>
